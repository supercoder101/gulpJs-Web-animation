...
export default class Triangles1 extends THREE.Mesh {
  constructor() {
    // Define Geometries
    ...
    for (var i = 0, ul = num; i < ul; i++) {
      const yBase = (Math.random() * 2 - 1) * -50
      const zBase = (Math.random() * 2 - 1) * 100
      ibaStartPositions.setXYZ(
        i,
        200,
        yBase + -60 + (Math.random() * 2 - 1) * 10,
        zBase
      );
      ibaEndPositions.setXYZ(
        i,
        -200,
        yBase + -90 + (Math.random() * 2 - 1) * 10,
        zBase
      );
      ibaRotates.setXYZ(
        i,
